...
export class BadgeService {
 ...
 /* async updateBadge(data: { userId: string; challengeId: string; name: string; awardedAt?: Date }): Promise<IBadge> {
    const badge = new Badge({
        userId: data.userId,
        challengeId: data.challengeId,
        name: data.name,
        awardedAt: data.awardedAt || new Date(), // Valeur par défaut si non fournie
    });

    return await badge.save(); // Sauvegarde le document dans MongoDB
}*/
  ...
  async getLeaderboard(): Promise<{ userId: string; score: number; badgeCount: number }[]> {
    const userScores: { [userId: string]: { score: number; badgeCount: number } } = {};
